...
class Sheet:
    def __init__(self, length: int, width: int, spacing: int):
        self.length = length
        self.width = width
        self.sheet = [[0] * width for _ in range(length)]
        self.space = spacing
        self.remaining = []
        
    def place_shape(self, x: int, y: int, shape) -> None:
    ...
    def is_valid_location(self, x: int, y: int, shape) -> bool:
        for i in range(shape.height):
            for j in range(shape.width):
                if (
                    x + i >= self.length
                    or y + j >= self.width
                    or (0 <= x + i < self.length and 0 <= y + j < self.width and self.sheet[x + i][y + j] == 1)
                ):
                    return False
        return True
    
    def find_empty_space(self, shape, start_x: int = 0, end_x: int = None, start_y: int = 0, end_y: int = None) -> Tuple[int, int]:
        ...
        for y in range(start_y, end_y):
            for x in range(start_x, end_x):
                if self.is_valid_location(x, y, shape):
                    return x, y
    ...
    def get_remaining_area(self) -> int:
        remaining_area = 0
        for row in self.sheet:
            remaining_area += row.count(0)
        return remaining_area
    
    def get_occupation_percentage(self) -> float:
        
        total_cells = self.length * self.width
        occupied_cells = total_cells - self.get_remaining_area()
